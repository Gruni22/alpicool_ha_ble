...
class FridgeCoordinator(ActiveBluetoothDataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(self, hass, logger, address: str, update_interval):
    ...
    def _notification_handler(self, sender, data: bytearray):
        ...
        while self._notification_buffer:
            ...
            if cmd == Request.QUERY:
                ...
                if decoded_status:
                    self.async_set_updated_data(decoded_status)
    ...
    async def async_send_command(self, packet: bytes) -> None:
        ...
        try:
            await bluetooth.async_ble_request(
                self.hass, 
                self.address, 
                lambda client: client.write_gatt_char(
                    FRIDGE_RW_CHARACTERISTIC_UUID, packet, response=self._write_requires_response
                )
            )
            # Fordere nach dem Senden ein sofortiges Update an.
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        if not hasattr(self, '_write_requires_response'):
        ...
        try:
            ...
            await asyncio.wait_for(self._status_updated_event.wait(), timeout=15)
    ...
    def _decode_status(self, payload: bytes) -> dict[str, Any]:
        try:
            ...
            decoded_data = {"locked": bool(payload[0]), "powered_on": bool(payload[1]), "run_mode": payload[2], "bat_saver": payload[3], "left_target": s_byte(payload[4]), "temp_max": s_byte(payload[5]), "temp_min": s_byte(payload[6]), "left_ret_diff": s_byte(payload[7]), "start_delay": s_byte(payload[8]), "unit": s_byte(payload[9]), "left_tc_hot": s_byte(payload[10]), "left_tc_mid": s_byte(payload[11]), "left_tc_cold": s_byte(payload[12]), "left_tc_halt": s_byte(payload[13]), "left_current": s_byte(payload[14]), "bat_percent": s_byte(payload[15]), "bat_vol_int": s_byte(payload[16]), "bat_vol_dec": s_byte(payload[17])}
            if len(payload) >= 28: 
                decoded_data.update({"right_target": s_byte(payload[18]), "right_ret_diff": s_byte(payload[21]), "right_tc_hot": s_byte(payload[22]), "right_tc_mid": s_byte(payload[23]), "right_tc_cold": s_byte(payload[24]), "right_tc_halt": s_byte(payload[25]), "right_current": s_byte(payload[26]), "running_status": s_byte(payload[27])})
