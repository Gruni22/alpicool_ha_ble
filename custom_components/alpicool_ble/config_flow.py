...
class AlpicoolConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if self._discovery_info:
            return self.async_create_entry(title=self._discovery_info.name, data={"address": self._discovery_info.address})
        ...
        if user_input is not None:
            ...
            if ble_device := async_ble_device_from_address(self.hass, address.upper(), True):
                ...
                self._abort_if_unique_id_configured()
                return self.async_create_entry(title=ble_device.name or address, data={"address": ble_device.address})
